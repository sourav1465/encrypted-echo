    ...
    <>
      <style>
        {`
          ...
          h2 { color: #38bdf8; font-size: 32px; margin-bottom: 10px; font-weight: 900; text-transform: uppercase; letter-spacing: 2px; }
          ...
          .login-btn { width: 100%; background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; border: none; border-radius: 14px; padding: 18px; font-size: 17px; font-weight: 800; cursor: pointer; transition: all 0.3s ease; text-transform: uppercase; letter-spacing: 2px; margin-top: 15px; }
          .login-btn:hover { filter: brightness(1.15); transform: translateY(-3px); box-shadow: 0 12px 25px rgba(16, 185, 129, 0.4); }
      ...
      <div className="login-wrapper">
        <div className="glass-card">
          <h2>Join Encrypted Echo</h2>
          <p style={{ color: '#94a3b8', margin: '0 0 30px 0', fontSize: '15px' }}>Start sharing your ideas today.</p>
          <form onSubmit={handleRegister}>
            <input type="text" placeholder="Choose a Username" className="styled-input" value={username} onChange={(e) => setUsername(e.target.value)} required />
            <div className="password-container">
              <input type={showPassword ? "text" : "password"} placeholder="Create a Password" className="styled-input" style={{ paddingRight: '50px' }} value={password} onChange={(e) => setPassword(e.target.value)} required />
          ...
          <div style={{ marginTop: '25px', fontSize: '15px', color: '#94a3b8' }}>
            Already have an account? <a href="/" style={{ color: '#10b981', textDecoration: 'none', fontWeight: 'bold' }}>Log in</a>
          </div>
        </div>
      </div>
    </>
